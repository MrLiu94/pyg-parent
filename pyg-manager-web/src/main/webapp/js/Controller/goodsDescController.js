...
app.controller('goodsDescController' ,function($scope,$controller   ,goodsDescService){	
    ...
    //    新增数据
    $scope.save=function () {
        goodsDescService.save($scope.entity).success(
            function (response) {
                if (response.success){
                    $scope.reloadList();//刷新
                }else {
    ...
    $scope.dele=function(){
        if(confirm('确定要删除吗？')){
            goodsDescService.dele($scope.selectList).success(
                function(response){
                    if(response.success){
                        $scope.reloadList();//刷新
                    }else{
    ...
    $scope.findPage=function (page,size) {
        goodsDescService.findPage(page,size,$scope.searchEntity).success(
            function (response) {
